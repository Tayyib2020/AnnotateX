...
function shouldShowRecoveryPending(task) {
  return task?.recovery?.pending === true
    && String(task?.payout?.status || "").toLowerCase() !== "refunded";
}
...
window.AnnotateX = {
  ...
  statusLabel, displayStatus, shouldShowRecoveryAction, shouldShowRecoveryPending, initializeTheme, initializeMobileMenu, getCurrentUser, requireDashboardRole,
